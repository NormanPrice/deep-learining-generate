...
def main(filename, seq_length, train_model_flag, output_path, pretrained_model_path, save_model_path):
    ...
    if args.fine_tune:
        if args.words_file is None:
            raise ValueError("You must provide a file with words for fine-tuning when using --fine_tune.")
        with open(args.words_file, 'r') as file:
            personalized_words = [line.strip() for line in file if line.strip()]
        model = fine_tune_on_words(model, personalized_words, char_indices, indices_char, seq_length, total_chars, args.learning_rate, args.epochs)

    generated_text = generate_text(args.length, args.temperature)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--words_file', type=str, help='File containing words for fine-tuning.')
